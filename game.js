...
// =====================================================================
const GAME = {
    day: 1, time: 10, energy: 100, stress: 0,
    location: 'apartamento',
    flags: {},
    ended: false
};
...
function modPlayer(energy, stress) {
    if (GAME.ended) return;
    GAME.energy = Math.max(0, Math.min(100, GAME.energy + energy));
    GAME.stress  = Math.max(0, Math.min(100, GAME.stress  + stress));
    document.getElementById('ui-energy').innerText = GAME.energy + '%';
    document.getElementById('bar-energy').style.width = GAME.energy + '%';
    document.getElementById('ui-stress').innerText = GAME.stress + '%';
    document.getElementById('bar-stress').style.width = GAME.stress + '%';
    if (GAME.stress >= 85) log('<b>⚠ ESTRÉS CRÍTICO</b>. Descansa antes de actuar.', 'warn');
}
...
function renderActionPanel() {
    ...
    if (activeNpcId) {
        ...
        if (!npc.met) {
            ...
            h += `<button class="act-btn" onclick="doAction('touch')">✋ Escalar contacto físico</button>`;
            h += `<button class="act-btn danger" onclick="doAction('libertine')">🎲 Dar el paso</button>`;
            if (npc.stats.afecto > 50) h += `<button class="act-btn success" onclick="doAction('compliment')">💝 Dar un cumplido sincero</button>`;
            if (npc.stats.complicidad > 40) h += `<button class="act-btn danger" onclick="doAction('provoke')">😈 Provocar / Jugar</button>`;
        }
        h += `<button class="act-btn span2" style="color:var(--muted);" onclick="activeNpcId=null;renderActionPanel()">← Dejar de interactuar</button>`;
    } else {
        if (GAME.location === 'apartamento') {
            h += `<button class="act-btn span2 success" onclick="doRest()">😴 Dormir / Descansar (+Energía, -Estrés)</button>`;
            h += `<button class="act-btn span2" onclick="doOsint()">🔍 Cotillear redes sociales (OSINT)</button>`;
        }
        h += `<button class="act-btn span2" onclick="doWait()">⏱ Esperar 1 hora aquí</button>`;
    }
...
function doAction(type) {
    ...
    if (GAME.stress >= 85) {
        log(`<b>ESTRÉS ALTO:</b> Actúas de manera torpe. ${npc.name} se incomoda.`, 'warn');
        modNpc(npc.id, { sospecha: +12, afecto: -8 });
        modPlayer(0, -20); advanceTime(1); return;
    }
    modPlayer(-5, 4);
    ...
    switch(type) {
        ...
        case 'compliment':
            const compliments = {
                elena: `"Hay algo en cómo miras el mundo que me parece increíble. No es algo que se finj." Elena sonríe de verdad.`,
                sofia: `"Debajo de toda esa armadura hay alguien que merece mucho más de lo que acepta." Sofía no sabe si molestarse o agradecerte.`,
                luna: `"Tus fotos te delatan: ves cosas que el resto no ve. Eso no se aprende." Luna baja la cámara.`,
                vera: `"Eres la persona más honesta que he conocido y eso me parece brutalmente valioso." Vera deja de revisar su teléfono.`
            };
            log(compliments[npc.id] || `Le dices algo sincero y específico.`, 'event');
            modNpc(npc.id, { afecto: +15, autoestima: +10, atraccion: +5 }); break;

        case 'provoke':
            const provokes = {
                elena: `"Para alguien que dice estar bien, te pasas mucho tiempo en el mismo café con el mismo café frío." Elena ríe a su pesar.`,
                sofia: `"¿Cuándo fue la última vez que hiciste algo que no podías contarle a nadie?" Sofía arquea una ceja.`,
                luna: `"¿Por qué fotografías a todo el mundo menos a ti misma?" Luna baja la cámara despacio.`,
                vera: `"Llevas quince minutos mirando el móvil cada dos minutos. No es trabajo, ¿verdad?" Vera se sonroja ligeramente.`
            };
            log(provokes[npc.id] || `La provocas suavemente. Toca un nervio.`, 'event');
            modNpc(npc.id, { complicidad: +12, atraccion: +10, sospecha: +5 }); break;
    }
    advanceTime(1);
}
...
function replyChat(npcId, type) {
    const npc = NPCS[npcId];
    npc.phoneState.chatHistory.push({ from:'player', text: 'Contesté 😊' });
    npc.phoneState.pendingMsg = null;
    npc.phoneState.readButNoReply = false;
    npc.phoneState.ignoreTime = 0;
    modNpc(npcId, { atraccion: +3, sospecha: -2 });
